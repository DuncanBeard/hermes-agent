...
class TestResolveProviderClientUniversalModelFallback:
    @pytest.mark.parametrize("explicit_model,expected", [("", "main-model"), ("chosen-model", "chosen-model")])
    def test_custom_model_fallback_preserves_explicit_override(self, explicit_model, expected):
        with patch("agent.auxiliary_client._read_main_model", return_value="main-model"), \
            ...
            client, model = resolve_provider_client(
                "custom", explicit_model, explicit_base_url="https://example.test/v1",
...
class TestGetTextAuxiliaryClient:
    ...
    def test_custom_endpoint_uses_codex_wrapper_when_runtime_requests_responses_api(self):
        with patch("hermes_cli.runtime_provider.resolve_runtime_provider",
                   return_value={"provider": "custom", "base_url": "https://api.openai.com/v1",
                                 "api_key": "sk-test", "api_mode": "codex_responses"}), \
...
class TestVisionClientFallback:
    ...
    def test_vision_auto_includes_active_provider_when_configured(self, monkeypatch):
        ...
        with (
            ...
            patch("agent.auxiliary_client.resolve_provider_client", return_value=(MagicMock(), "claude-sonnet-4")),
...
class TestGetProviderChain:
    ...
    def test_returns_supported_entries(self):
        chain = _get_provider_chain()
        assert len(chain) == 3
        labels = [label for label, _ in chain]
        assert labels == ["nous", "custom", "copilot"]
...
class TestTryPaymentFallback:
    ...
    def test_skips_failed_provider(self):
        ...
        with patch("agent.auxiliary_client._try_nous", return_value=(mock_client, "nous-model")), \
            ...
            client, model, label = _try_payment_fallback("copilot", task="compression")
    ...
    def test_codex_not_in_fallback_chain(self):
        ...
        with patch("agent.auxiliary_client._try_nous", return_value=(None, None)), \
             ...
             patch("agent.auxiliary_client.resolve_provider_client", return_value=(None, None)), \
             patch("agent.auxiliary_client._read_main_provider", return_value="auto"):
            client, model, label = _try_payment_fallback("copilot")
        assert client is None
        assert model is None
        assert label == ""
...
class TestStaleBaseUrlWarning:
    ...
    def test_warns_when_openai_base_url_set_with_named_provider(self, monkeypatch, caplog):
        ...
        with patch("agent.auxiliary_client.resolve_provider_client", return_value=(None, None)), patch("agent.auxiliary_client._read_main_provider", return_value="nous"), \
...
class TestAuxUnhealthyCache:
    ...
    def test_call_llm_marks_provider_unhealthy_on_402(self, monkeypatch):
        ...
        primary_client = MagicMock()
        # The retained account must be quarantined after its billing failure.
        ...
        with patch("agent.auxiliary_client._get_cached_client",
             ...
             patch("agent.auxiliary_client._resolve_task_provider_model",
                    return_value=("nous", "google/gemini-3-flash-preview", None, None, None)), \
             patch("agent.auxiliary_client._try_payment_fallback",
                    return_value=(nous_client, "n-model", "copilot")) as automatic_fallback, \
            ...
            # Explicit accounts do not auto-hop to another account on exhaustion.
            with pytest.raises(Exception, match="Payment Required"):
                call_llm(
                    task="compression",
                    messages=[{"role": "user", "content": "hi"}],
                )
            automatic_fallback.assert_not_called()
            # The terminal billing error still quarantines the selected account.
...
class TestCompressionFallbackContextFilter:
    ...
    def test_configured_chain_skips_too_small_candidate_for_compression(self, monkeypatch):
        ...
        entries = [
            self._make_chain_entry("custom:small-provider", "tiny-8k"),
            self._make_chain_entry("custom:big-provider", "huge-1m"),
        ]

        def fake_resolve(entry):
            if entry["model"] == "tiny-8k":
        ...
        assert "custom:big-provider" in label
    ...
    def test_same_provider_same_model_still_skipped(self, monkeypatch):
        ...
        entries = [
            self._make_chain_entry("custom", "deepseek-ai/deepseek-v4-pro"),
        ]
